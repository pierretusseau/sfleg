      ...
      <Stack spacing={2}>
        <Box sx={{width: '100%'}}>
          <FormControl fullWidth>
            ...
            >
              {weapons.quality.map(opt => <MenuItem value={opt.id} key={opt.id}>
        ...
        <Box>
          <FormControl fullWidth>
            ...
            >
              {weapons.modifier1.map(opt => <MenuItem value={opt.id} key={opt.id}>
                <span style={{textTransform: "uppercase"}}>{opt.name}</span>
                &nbsp;
        ...
        <Box>
          <FormControl fullWidth>
            ...
            >
              {weapons.modifier2.map((opt, i) => <MenuItem value={opt.id} key={`${opt.id}-${i}`}>
                <span style={{textTransform: "uppercase"}}>{opt.name}</span>
                &nbsp;
                <span style={{fontSize: "80%", fontWeight: "bold"}}>({opt.desc})</span>
              </MenuItem>)}
        ...
        <Box>
          <FormControl fullWidth>
            ...
            >
              {weapons.modifier3.map((opt, i) => <MenuItem value={opt.id} key={`${opt.id}-${i}`}>
                <span style={{textTransform: "uppercase"}}>{opt.name}</span>
                &nbsp;
                <span style={{fontSize: "80%", fontWeight: "bold"}}>({opt.desc})</span>
              </MenuItem>)}
